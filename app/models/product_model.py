...
class ProductTable(Base):
    ...
    product_description = Column(
        name="description", type_=String(255), nullable=False, unique=False
    )
    product_type = Column(
        name="type", type_=SQLAlchemyEnum(ProductType), nullable=False
    )
    price = Column(name="price", type_=Numeric(10, 2), nullable=False)
    pricing_unit = Column(
        name="pricing_unit", type_=SQLAlchemyEnum(PricingUnit), nullable=False
    )
    product_status = Column(
        name="status",
        type_=SQLAlchemyEnum(ProductStatus),
        nullable=False,
