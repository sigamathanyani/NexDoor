...
from app.services.product_service import create_product, delete_single_product, get_all_products, get_single_product, update_single_product
...
def add_product(
    ...
    return create_product(product_data, db=db, current_user=current_user,s3_client=s3_client)
...
def get_products_route(
    ...
    return get_all_products(db=db,s3_client=s3_client,)
...
def update_product_route(
    ...
    product = update_single_product(product_updated=product_updated,db=db, product_id=product_id,current_user=current_user)
    if product is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail={"message": "You cannot perform this operation "})
    return product
...
def delete_product_route(
    ...
    product = delete_single_product(db=db, product_id=product_id,current_user=current_user)
    if product is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail={"message": "You cannot perform this operation "})
    return product